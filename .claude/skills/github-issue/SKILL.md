...
File a structured GitHub issue (bug report or feature request) for OpsClaw interactively from Claude Code.

## When to Use

Trigger when the user wants to file a GitHub issue, report a bug, or request a feature for OpsClaw. Keywords: "file issue", "report bug", "feature request", "open issue", "create issue", "github issue".

## Instructions

You are filing a GitHub issue against the OpsClaw repository using structured issue forms. Follow this workflow exactly.
...
cargo metadata --format-version=1 --no-deps 2>/dev/null | jq -r '.packages[] | select(.name=="opsclaw") | .version' 2>/dev/null   # OpsClaw version
...
- **Use neutral project-scoped placeholders** per OpsClaw's privacy contract.
