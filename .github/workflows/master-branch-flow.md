...
OpsClaw uses a single default branch: `master`. All contributor PRs target `master` directly. There is no `dev` or promotion branch.

Current maintainers with PR approval authority: `DCPRevere`, `DCPRevere`, and `DCPRevere`.
...
4. Maintainer (`DCPRevere`, `DCPRevere`, or `DCPRevere`) merges PR once checks and review policy are satisfied.
