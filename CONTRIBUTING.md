# Contributing to OpsClaw

Thanks for your interest in contributing to OpsClaw! This guide will help you get started.

---

## Contributor Agreement

**By submitting a contribution (pull request, patch, or any other code or content),
you irrevocably assign all copyright and related rights in your contribution to
D C P Revere.** You confirm that you are the original author and have the right
to make this assignment. See `NOTICE` for the full contributor agreement.
...
**Background:** OpsClaw previously used `main` in some documentation and scripts, which caused 404 errors, broken CI refs, and contributor confusion (see [#2929](https://github.com/DCPRevere/opsclaw/issues/2929), [#3061](https://github.com/DCPRevere/opsclaw/issues/3061), [#3194](https://github.com/DCPRevere/opsclaw/pull/3194)). As of March 2026, all references have been corrected, stale branches cleaned up, and the `main` branch permanently deleted.
...
1. **Find an issue.** Look for issues labeled [`good first issue`](https://github.com/DCPRevere/opsclaw/issues?q=is%3Aissue+is%3Aopen+label%3A%22good+first+issue%22) — these are scoped for newcomers and include context to get moving quickly.
...
4. **Start with Track A.** OpsClaw uses three [collaboration tracks](#collaboration-tracks-risk-based) (A/B/C) based on risk. First-time contributors should target **Track A** (docs, tests, chore) — these require lighter review and are the fastest path to a merged PR.
...
OpsClaw supports layered secret management for local development and CI hygiene.
...
2. **Config file** (`~/.opsclaw/config.toml`)
    - Persistent setup for long-term use
    - When `secrets.encrypt = true` (default), secret values are encrypted before save
    - Secret key is stored at `~/.opsclaw/.secret_key` with restricted permissions
    - Use `opsclaw onboard` for guided setup
...
3. Generic env vars (`OPSCLAW_API_KEY`, `API_KEY`)

Provider/model config overrides:

- `OPSCLAW_PROVIDER` / `PROVIDER`
- `OPSCLAW_MODEL`
...
- `~/.opsclaw/.secret_key` or similar key files
...
- If identity-like wording is required, use OpsClaw-centric labels only (for example: `OpsClawAgent`, `OpsClawOperator`, `opsclaw_user`).
...
- **Identity normalization**: when identity traits are unavoidable, use OpsClaw/project-native roles instead of personal or real-world identities.
...
OpsClaw's architecture is built on **traits** — every subsystem is swappable. This means contributing a new integration is as simple as implementing a trait and registering it in the factory function.
...
- **Identity-like labels**: if unavoidable, use OpsClaw-native identifiers only (`OpsClawAgent`, `opsclaw_user`, `opsclaw_node`).
...
- **Good identity-like label**: `OpsClawAgent`, `opsclaw_user`, `opsclaw_node`
...
- [ ] Any required identity-like wording uses OpsClaw/project-native labels only
