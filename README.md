...
Target config schema, encrypted secret store, SshTool, discovery scan, notification channel setup, basic monitoring loop, `opsclaw setup` CLI.

### Phase 2: Deeper Monitoring & Kubernetes
Event streams (Docker events, journalctl), log source integration (Elasticsearch, Loki, CloudWatch), KubeTool, external probes, baseline learning.

### Phase 3: Diagnosis & Remediation
Incident memory, living runbooks, autonomy enforcement mapped to ZeroClaw's security policy.

### Phase 4: Escalation & Reporting
Escalation engine with on-call rotation, daily digests, weekly summaries, trend analysis.

### Phase 5: Polish
Rebrand from ZeroClaw, web dashboard, database diagnostic tools, documentation.

## Pricing

OpsClaw is open-source (Apache-2.0 / MIT dual-licensed). Bring your own API keys.

| Tier | Price | What you get |
|---|---|---|
| **Free** | £0 | 1 target, observe mode, community support |
| **Pro** | £19/month | Unlimited targets, all autonomy levels, incident memory, runbooks, email support |
| **Team** | £49/month | Multi-user, RBAC, shared runbooks, audit log export, priority support |
| **Managed** | £49/month | We run OpsClaw for you. Hosted instance, managed updates, dedicated support |

## Getting Started

```bash
# Install
curl -sSf https://opsclaw.dev/install.sh | sh

# Set up your first target
opsclaw setup

# Or configure manually
opsclaw onboard        # LLM provider setup (inherited from ZeroClaw)
