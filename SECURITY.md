...
2. **GitHub**: Use [GitHub Security Advisories](https://github.com/DCPRevere/opsclaw/security/advisories/new)
...
OpsClaw implements defense-in-depth security:
...
OpsClaw Docker images follow CIS Docker Benchmark best practices:
...
docker inspect --format='{{.Config.User}}' opsclaw
# Expected: 65534:65534

# Run with read-only filesystem (production hardening)
docker run --read-only -v /path/to/workspace:/workspace opsclaw gateway
```
