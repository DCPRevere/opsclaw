...
//!
//! 1. `OPSCLAW_AARDVARK_LIB` environment variable (full path to `aardvark.so`)
...
//! # Safety
//!
//! This crate is the **only** place in OpsClaw where `unsafe` is permitted.
    ...
    AARDVARK_LIB
        .get_or_init(|| {
            let candidates: Vec<PathBuf> = vec![
                ...
                std::env::var("OPSCLAW_AARDVARK_LIB")
                    .ok()
                    .map(PathBuf::from)
                    .unwrap_or_default(),
            ...
            if !tried_any {
                eprintln!("[aardvark-sys] no library candidates found; set OPSCLAW_AARDVARK_LIB or place aardvark.so next to the binary");
            }
            None
        })
        .as_ref()
...
pub enum AardvarkError {
    ...
    #[error("aardvark.so not found — set OPSCLAW_AARDVARK_LIB or place it next to the binary")]
    LibraryNotFound,
}
