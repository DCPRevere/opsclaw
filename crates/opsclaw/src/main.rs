...
/// `OpsClaw` - Autonomous SRE agent.
#[derive(Parser, Debug)]
#[command(name = "opsclaw")]
#[command(author = "D C P Revere")]
#[command(version)]
#[command(about = "Autonomous SRE agent — monitors your servers and fixes them while you sleep.", long_about = None)]
struct Cli {
...
Examples:
  opsclaw acp                        # start ACP server
  opsclaw acp --max-sessions 5       # limit concurrent sessions")]
    Acp {
...
Examples:
  ...
  opsclaw channel bind-telegram opsclaw_user
...
Examples:
  ...
  opsclaw config target add                          # add a target (wizard)")]
    ...
    #[command(long_about = "\
Check for and apply OpsClaw updates.
...
Examples:
  opsclaw update                      # download and install latest
  opsclaw update --check              # check only, don't install
  opsclaw update --force              # install without confirmation
  opsclaw update --version 0.6.0      # install specific version")]
    Update {
    ...
    #[command(long_about = "\
Run diagnostic self-tests to verify the OpsClaw installation.
...
Examples:
  opsclaw self-test             # full suite
  opsclaw self-test --quick     # quick checks only (no network)")]
    SelfTest {
...
Examples:
  ...
  opsclaw completions fish > ~/.config/fish/completions/opsclaw.fish")]
    ...
    #[command(long_about = "\
Launch the OpsClaw companion desktop app.
...
Examples:
  opsclaw desktop              # launch the companion app
  opsclaw desktop --install    # download and install it")]
    Desktop {
...
async fn main() -> Result<()> {
    ...
    // or set OPSCLAW_INTERACTIVE=1 to force interactive mode when TTY
    ...
    } = &cli.command
    {
        ...
        let env_interactive = std::env::var("OPSCLAW_INTERACTIVE").as_deref() == Ok("1");
    ...
    let ops_config = {
        let config_path = ops::setup::opsclaw_config_path()?;
        ops::setup::load_existing_config(&config_path)
    };
    ...
    match cli.command {
        ...
        Commands::Desktop {
            ...
            let download_url = "https://www.opsclawlabs.ai/download";

            if do_install {
                println!("Download the OpsClaw companion app:");
                println!();
                #[cfg(target_os = "macos")]
                {
                    ...
                    println!("  brew install --cask opsclaw");
                }
                #[cfg(target_os = "linux")]
                {
            ...
            let desktop_bin = {
                ...
                // 1. macOS: check /Applications/OpsClaw.app
                #[cfg(target_os = "macos")]
                {
                    let app_paths = [
                        PathBuf::from("/Applications/OpsClaw.app/Contents/MacOS/OpsClaw"),
                        PathBuf::from(std::env::var("HOME").unwrap_or_default())
                            .join("Applications/OpsClaw.app/Contents/MacOS/OpsClaw"),
                    ];
                ...
                if found.is_none() {
                    if let Ok(exe) = std::env::current_exe() {
                        let sibling = exe.with_file_name("opsclaw-desktop");
                        if sibling.is_file() {
                            found = Some(sibling);
                        }
                    }
                ...
                // 3. ~/.cargo/bin/opsclaw-desktop or ~/.local/bin/opsclaw-desktop
                if found.is_none() {
                    if let Some(home) = std::env::var_os("HOME") {
                        let home = PathBuf::from(home);
                        for dir in &[".cargo/bin", ".local/bin"] {
                            let candidate = home.join(dir).join("opsclaw-desktop");
                ...
                if found.is_none() {
                    if let Ok(path) = which::which("opsclaw-desktop") {
                        found = Some(path);
                    }
                }
            ...
            match desktop_bin {
                Some(bin) => {
                    println!("Launching OpsClaw companion app...");
                ...
                None => {
                    println!("OpsClaw companion app is not installed.");
                    println!();
                    println!("  Download it at: {download_url}");
                    println!("  Or run: opsclaw desktop --install");
                    println!();
...
mod tests {
    ...
    fn onboard_cli_parses_quick_flag() {
        let cli = Cli::try_parse_from(["opsclaw", "onboard", "--quick"])
    ...
    fn onboard_cli_quick_and_channels_only_conflict() {
        ...
        let cli = Cli::try_parse_from(["opsclaw", "onboard", "--quick", "--channels-only"]);
        assert!(
            cli.is_ok(),
