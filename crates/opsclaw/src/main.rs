...
pub use zeroclaw::{
    A2aCommands, ChannelCommands, CronCommands, GatewayCommands, HardwareCommands,
    IntegrationCommands, MemoryCommands, MigrateCommands, PeripheralCommands, SecretCommands,
...
/// `OpsClaw` - Zero overhead. Zero compromise. 100% Rust.
#[derive(Parser, Debug)]
#[command(name = "opsclaw")]
#[command(author = "D C P Revere")]
#[command(version)]
...
Examples:
  opsclaw agent                              # interactive session
  opsclaw agent -m \"Summarize today's logs\"  # single message
  opsclaw agent -p anthropic --model claude-sonnet-4-20250514
  opsclaw agent --peripheral nucleo-f401re:/dev/ttyACM0")]
    Agent {
...
Examples:
  opsclaw gateway start              # start gateway
  opsclaw gateway restart            # restart gateway
  opsclaw gateway get-paircode       # show pairing code")]
    Gateway {
        #[command(subcommand)]
    ...
    #[command(long_about = "\
Interact with remote A2A-compliant agents.

The A2A protocol enables agent-to-agent communication over HTTP \
using JSON-RPC 2.0. Configure peers in [a2a.peers] in config.toml.

Examples:
  opsclaw a2a discover https://agent.example.com
  opsclaw a2a send my-peer 'Summarize logs'
  opsclaw a2a status my-peer <task-id>
  opsclaw a2a server
  opsclaw a2a peers")]
    A2a {
        #[command(subcommand)]
...
Launches the full OpsClaw runtime: gateway server, all configured \
channels (Telegram, Discord, Slack, etc.), heartbeat monitor, and \
the cron scheduler. This is the recommended way to run OpsClaw in \
production or as an always-on assistant.

Use 'opsclaw service install' to register the daemon as an OS \
service (systemd/launchd) for auto-start on boot.

Examples:
  opsclaw daemon                   # use config defaults
  opsclaw daemon -p 9090           # gateway on port 9090
  opsclaw daemon --host 127.0.0.1  # localhost only")]
    Daemon {
    ...
    /// Examples:
    /// - `opsclaw estop`
    /// - `opsclaw estop --level network-kill`
    /// - `opsclaw estop --level domain-block --domain "*.chase.com"`
    /// - `opsclaw estop --level tool-freeze --tool shell --tool browser`
    /// - `opsclaw estop status`
    /// - `opsclaw estop resume --network`
    /// - `opsclaw estop resume --domain "*.chase.com"`
    /// - `opsclaw estop resume --tool shell`
    Estop {
        #[command(subcommand)]
        estop_command: Option<EstopSubcommands>,

        /// Level used when engaging estop from `opsclaw estop`.
        #[arg(long, value_enum)]
...
Examples:
  opsclaw cron list
  opsclaw cron add '0 9 * * 1-5' 'Good morning' --tz America/New_York --agent
  opsclaw cron add '*/30 * * * *' 'Check system health' --agent
  opsclaw cron add '*/5 * * * *' 'echo ok'
  opsclaw cron add-at 2025-01-15T14:00:00Z 'Send reminder' --agent
  opsclaw cron add-every 60000 'Ping heartbeat'
  opsclaw cron once 30m 'Run backup in 30 minutes' --agent
  opsclaw cron pause <task-id>
  opsclaw cron update <task-id> --expression '0 8 * * *' --tz Europe/London")]
    Cron {
        #[command(subcommand)]
...
Add, remove, list, send, and health-check channels that connect OpsClaw \
to messaging platforms. Supported channel types: telegram, discord, \
slack, whatsapp, matrix, imessage, email.

Examples:
  opsclaw channel list
  opsclaw channel doctor
  opsclaw channel add telegram '{\"bot_token\":\"...\",\"name\":\"my-bot\"}'
  opsclaw channel remove my-bot
  opsclaw channel bind-telegram zeroclaw_user
  opsclaw channel send 'Alert!' --channel-id telegram --recipient 123456789")]
    Channel {
        #[command(subcommand)]
...
Examples:
  opsclaw hardware discover
  opsclaw hardware introspect /dev/ttyACM0
  opsclaw hardware info --chip STM32F401RETx")]
    Hardware {
        #[command(subcommand)]
...
Examples:
  opsclaw peripheral list
  opsclaw peripheral add nucleo-f401re /dev/ttyACM0
  opsclaw peripheral add rpi-gpio native
  opsclaw peripheral flash --port /dev/cu.usbmodem12345
  opsclaw peripheral flash-nucleo")]
    Peripheral {
        #[command(subcommand)]
...
Examples:
  opsclaw memory stats
  opsclaw memory list
  opsclaw memory list --category core --limit 10
  opsclaw memory get <key>
  opsclaw memory clear --category conversation --yes")]
    Memory {
        #[command(subcommand)]
...
Examples:
  ...
  opsclaw monitor --once                 # single check, then exit")]
    ...
    /// Run external probes (HTTP, TCP, DNS, TLS) against targets
    #[command(long_about = "\
Run external probes for configured targets.

Executes HTTP, TCP, DNS, and TLS certificate probes defined in the target \
config (plus auto-discovered probes from the latest snapshot). Use --url \
for a quick one-off HTTP probe without any config.

Examples:
  opsclaw probe --target sacra         # run all probes for a target
  opsclaw probe --all                  # run all probes for all targets
  opsclaw probe --url https://example.com  # quick one-off HTTP probe")]
    Probe {
        /// Target name to probe (from config [[targets]])
        #[arg(long)]
        target: Option<String>,
        /// Probe all configured targets
        #[arg(long)]
        all: bool,
        /// Quick one-off HTTP probe URL
        #[arg(long)]
        url: Option<String>,
    },
...
Auto-discovers log sources from the saved discovery snapshot (or runs \
a fresh scan). Supports filtering by source type and minimum severity.
...
Examples:
  ...
  opsclaw watch --target sacra         # single target")]
    ...
    /// Show or reset metric baselines for monitored targets
    #[command(long_about = "\
Show or reset metric baselines for monitored targets.

Displays rolling statistics (mean, stddev, trend) for each tracked \
metric on the specified target. Use --reset to clear baseline data.

Examples:
  opsclaw baseline                         # all targets
  ...
  opsclaw baseline --target sacra --reset  # clear one target")]
    ...
    /// View or clear the dry-run log
    #[command(long_about = "\
Show what OpsClaw would have done in dry-run mode.

The dry-run log is written to ~/.opsclaw/dry-run.log whenever a target \
runs in dry-run autonomy mode and a write command is proposed.

Examples:
  opsclaw dry-run-log                    # print entire log
  opsclaw dry-run-log --tail 20          # last 20 lines
  opsclaw dry-run-log --clear            # delete the log")]
    DryRunLog {
...
Incidents are recorded automatically during monitoring when the LLM \
diagnoses an issue. Use this command to review past incidents, search \
by keyword, or mark an incident as resolved.

Examples:
  opsclaw incidents                              # list recent incidents (all targets)
...
Examples:
  source <(opsclaw completions bash)
  opsclaw completions zsh > ~/.zfunc/_opsclaw
  opsclaw completions fish > ~/.config/fish/completions/opsclaw.fish")]
    Completions {
        /// Target shell
        #[arg(value_enum)]
...
use opsclaw::ops_cli::RunbookActions;
...
async fn main() -> Result<()> {
    ...
    for (opsclaw_var, zeroclaw_var) in &[
        ...
        if let Ok(val) = std::env::var(opsclaw_var) {
            if std::env::var(zeroclaw_var).is_err() {
                ...
                #[allow(deprecated)]
                unsafe { std::env::set_var(zeroclaw_var, &val) };
            }
        }
    }
    ...
    if let Some(config_dir) = &cli.config_dir {
        ...
        #[allow(deprecated)]
        unsafe { std::env::set_var("ZEROCLAW_CONFIG_DIR", config_dir); } // upstream compat
    }
    ...
    // `opsclaw onboard --api-key …` both take the fast path, while a bare
    // `opsclaw onboard` in a terminal launches the wizard.
    ...
    } = &cli.command
    {
        ...
        if reinit {
            let (opsclaw_dir, _) =
                zeroclaw::config::schema::resolve_runtime_dirs_for_onboarding().await?;

            if opsclaw_dir.exists() {
                let timestamp = chrono::Local::now().format("%Y%m%d%H%M%S");
                let backup_dir = format!("{}.backup.{}", opsclaw_dir.display(), timestamp);

                println!("⚠️  Reinitializing OpsClaw configuration...");
                println!("   Current config directory: {}", opsclaw_dir.display());
                println!(
                ...
                tokio::fs::rename(&opsclaw_dir, &backup_dir)
                    .await
                    .with_context(|| {
        ...
        if std::env::var("OPSCLAW_AUTOSTART_CHANNELS").as_deref() == Ok("1")
            || std::env::var("ZEROCLAW_AUTOSTART_CHANNELS").as_deref() == Ok("1") {
    ...
    if config.security.otp.enabled {
        ...
        if let Some(uri) = enrollment_uri {
            println!("Initialized OTP secret for OpsClaw.");
    ...
    match cli.command {
        ...
        Commands::Gateway { gateway_command } => {
            match gateway_command {
                Some(zeroclaw::GatewayCommands::Restart { port, host }) => {
                    ...
                    info!("🔄 Restarting OpsClaw Gateway on {addr}");
        ...
        Commands::Daemon { port, host } => {
            ...
            if port == 0 {
                info!("🧠 Starting OpsClaw Daemon on {host} (random port)");
            } else {
        ...
        Commands::Status => {
            ...
            let telegram_configured = config
                .notifications
                .as_ref()
                .map_or(false, |n| n.telegram_bot_token.is_some() && n.telegram_chat_id.is_some());
            println!(
            ...
            // Diagnosis config
            let diagnosis_key_present = config
                .diagnosis
                .api_key
                .is_some()
        ...
        Commands::Setup => opsclaw::ops::setup::run_opsclaw_setup().await,
        Commands::Logs {
            target,
            source,
            lines,
...
fn log_gateway_start(host: &str, port: u16) {
    if port == 0 {
        info!("🚀 Starting OpsClaw Gateway on {host} (random port)");
    } else {
        info!("🚀 Starting OpsClaw Gateway on {host}:{port}");
    }
}
                ...
                "gemini" => {
                    ...
                    .await
                    {
                        ...
                        Err(e) => {
                            ...
                            println!(
                                "Run `opsclaw auth paste-redirect --provider gemini --profile {profile}`"
                            );
                            return Ok(());
                        }
                    };
                ...
                "openai-codex" => {
                    ...
                    .await
                    {
                        ...
                        Err(e) => {
                            println!("Callback capture failed: {e}");
                            println!(
                                "Run `opsclaw auth paste-redirect --provider openai-codex --profile {profile}`"
                            );
                            return Ok(());
                        }
                    };
            ...
            match provider.as_str() {
                "openai-codex" => {
                    let pending = load_pending_oauth_login(config, "openai")?.ok_or_else(|| {
                        anyhow::anyhow!(
                            "No pending OpenAI login found. Run `opsclaw auth login --provider openai-codex` first."
                        )
                    })?;
                ...
                "gemini" => {
                    let pending = load_pending_oauth_login(config, "gemini")?.ok_or_else(|| {
                        anyhow::anyhow!(
                            "No pending Gemini login found. Run `opsclaw auth login --provider gemini` first."
                        )
                    })?;
            ...
            match provider.as_str() {
                "openai-codex" => {
                        ...
                        .await?
                    {
                        ...
                        None => {
                            bail!(
                                "No OpenAI Codex auth profile found. Run `opsclaw auth login --provider openai-codex`."
                            )
                        }
                    }
                }
                "gemini" => {
                        ...
                        .await?
                    {
                        ...
                        None => {
                            bail!(
                                "No Gemini auth profile found. Run `opsclaw auth login --provider gemini`."
                            )
                        }
                    }
                }
...
mod tests {
    ...
    fn completions_cli_parses_supported_shells() {
        for shell in ["bash", "fish", "zsh", "powershell", "elvish"] {
            let cli = Cli::try_parse_from(["opsclaw", "completions", shell])
    ...
    fn onboard_cli_accepts_force_flag() {
        let cli = Cli::try_parse_from(["opsclaw", "onboard", "--force"])
    ...
    fn onboard_cli_rejects_removed_interactive_flag() {
        ...
        assert!(Cli::try_parse_from(["opsclaw", "onboard", "--interactive"]).is_err());
    }

    #[test]
    fn onboard_cli_bare_parses() {
        let cli = Cli::try_parse_from(["opsclaw", "onboard"]).expect("bare onboard should parse");
    ...
    fn cli_parses_estop_default_engage() {
        let cli = Cli::try_parse_from(["opsclaw", "estop"]).expect("estop command should parse");
    ...
    fn cli_parses_estop_resume_domain() {
        let cli = Cli::try_parse_from(["opsclaw", "estop", "resume", "--domain", "*.chase.com"])
    ...
    fn agent_command_parses_with_temperature() {
        let cli = Cli::try_parse_from(["opsclaw", "agent", "--temperature", "0.5"])
    ...
    fn agent_command_parses_without_temperature() {
        let cli = Cli::try_parse_from(["opsclaw", "agent", "--message", "hello"])
