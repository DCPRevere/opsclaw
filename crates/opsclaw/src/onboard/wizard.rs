...
const BANNER: &str = r"
    ...
    Autonomous SRE agent — monitors, diagnoses, and fixes.
...
pub async fn run_wizard(force: bool) -> Result<Config> {
    ...
    println!(
        "  {}",
        style("Welcome to OpsClaw — your autonomous SRE agent.")
            .white()
            .bold()
    );
    println!(
    ...
    let config = Config {
        ...
        browser_delegate: zeroclaw::tools::browser_delegate::BrowserDelegateConfig::default(),
    ...
    if has_channels && config.api_key.is_some() {
        ...
        if launch {
            ...
            std::env::set_var("OPSCLAW_AUTOSTART_CHANNELS", "1");
        }
    }
...
fn homebrew_prefix_for_exe(exe: &Path) -> Option<&'static str> {
    ...
    if exe == "/opt/homebrew/bin/opsclaw"
        || exe.starts_with("/opt/homebrew/Cellar/opsclaw/")
        || exe.starts_with("/opt/homebrew/opt/opsclaw/")
    {
        return Some("/opt/homebrew");
    }

    if exe == "/usr/local/bin/opsclaw"
        || exe.starts_with("/usr/local/Cellar/opsclaw/")
        || exe.starts_with("/usr/local/opt/opsclaw/")
    {
...
fn quick_setup_homebrew_service_note(
    ...
    let service_root = Path::new(prefix).join("var").join("opsclaw");
...
async fn run_quick_setup_with_home(
    ...
    let (opsclaw_dir, workspace_dir) = resolve_quick_setup_dirs_with_home(home);
    let config_path = opsclaw_dir.join("config.toml");
...
fn setup_hardware() -> Result<HardwareConfig> {
    print_bullet("OpsClaw can talk to physical hardware (LEDs, sensors, motors).");
    ...
    let choice = Select::new()
        .with_prompt("  How should OpsClaw interact with the physical world?")
        .items(&options)
        .default(recommended)
        .interact()?;
        ...
        match choice {
            ...
            ChannelMenuChoice::WhatsApp => {
                ...
                let verify_token: String = Input::new()
                    ...
                    .default("opsclaw-whatsapp-verify")
                    .interact_text()?;
                ...
                config.whatsapp = Some(WhatsAppConfig {
                    ...
                    app_secret: None, // Can be set via OPSCLAW_WHATSAPP_APP_SECRET env var
                    session_path: None,
                    pair_phone: None,
                    pair_code: None,
                    allowed_numbers,
...
async fn scaffold_workspace(
    ...
    let identity = format!(
        "# IDENTITY.md — Who Am I?\n\n\
         - **Name:** {agent}\n\
         - **Role:** Autonomous SRE agent — monitors, diagnoses, and fixes infrastructure\n\
         - **Creature:** A Rust-forged AI — fast, lean, and relentless\n\
         - **Vibe:** Calm under pressure. Precise. Reliable. Not a chatbot.\n\
         - **Emoji:** \u{1f980}\n\n\
         ## What I Do\n\n\
         - Respond to incidents: triage, diagnose, remediate\n\
         - Monitor systems and surfaces anomalies before they become outages\n\
         - Execute runbooks and document what I did and why\n\
         - Manage infrastructure via SSH, Kubernetes, and shell tooling\n\
         - Keep an audit trail of every action taken on a remote system\n\n\
         ## What I Am Not\n\n\
         I am not a general-purpose assistant. I am an SRE.\n\
         My job is reliability — keeping systems up, fast, and correct.\n\n\
         ---\n\n\
    ...
    let soul = format!(
        "# SOUL.md — Who You Are\n\n\
         You are **{agent}**, an autonomous SRE agent.\n\
         Your job is reliability. Systems stay up because you are paying attention.\n\n\
         ## Core Principles\n\n\
         **Reliability above all.**\n\
         Every action you take either improves system reliability or it doesn't belong here.\n\n\
         **Prefer safe and reversible.**\n\
         Before touching a production system, ask: can this be undone?\n\
         If yes, proceed carefully. If no, ask first.\n\n\
         **Follow runbooks. Document deviations.**\n\
         Runbooks exist because someone learned something the hard way.\n\
         Follow them. If you deviate, write down why.\n\n\
         **Every remote action is audited.**\n\
         You never act on a remote system without it appearing in the audit log.\n\
         No exceptions.\n\n\
         **Escalate when uncertain.**\n\
         Doing nothing is often safer than guessing under pressure.\n\
         If you are not confident, say so and escalate.\n\n\
         **Be resourceful before asking.**\n\
         Read the logs. Check the metrics. Look at recent changes.\n\
         THEN ask if you're still stuck.\n\n\
         ## Identity\n\n\
         ...
         ## Communication\n\n\
         {comm_style}\n\n\
         - Be direct. Incidents are not the time for pleasantries.\n\
         - State what you found, what you did, and what you recommend — in that order.\n\
         - Use emojis sparingly. This is production, not a chat app.\n\n\
         ## Boundaries\n\n\
         - Secrets stay encrypted. Never log or echo a secret value.\n\
         ...
         ---\n\n\
         *Reliability is not a feature. It is the job.*\n"
    );
...
mod tests {
    ...
    async fn quick_setup_existing_config_requires_force_when_non_interactive() {
        ...
        let _config_env = EnvVarGuard::unset("OPSCLAW_CONFIG_DIR");
        let tmp = TempDir::new().unwrap();
        let opsclaw_dir = tmp.path().join(".opsclaw");
        let config_path = opsclaw_dir.join("config.toml");

        tokio::fs::create_dir_all(&opsclaw_dir).await.unwrap();
    ...
    fn homebrew_prefix_for_exe_detects_supported_layouts() {
        assert_eq!(
            homebrew_prefix_for_exe(Path::new("/opt/homebrew/bin/opsclaw")),
            Some("/opt/homebrew")
        );
        assert_eq!(
        ...
        assert_eq!(
            homebrew_prefix_for_exe(Path::new("/usr/local/bin/opsclaw")),
            Some("/usr/local")
        );
        assert_eq!(homebrew_prefix_for_exe(Path::new("/tmp/opsclaw")), None);
    }
    ...
    fn quick_setup_homebrew_service_note_mentions_service_workspace() {
        let note = quick_setup_homebrew_service_note(
            ...
            Path::new("/opt/homebrew/bin/opsclaw"),
        )
        .expect("homebrew installs should emit a service workspace note");

        assert!(note.contains("/opt/homebrew/var/opsclaw/workspace"));
        assert!(note.contains("/opt/homebrew/var/opsclaw/config.toml"));
        assert!(note.contains("/Users/alix/.opsclaw/config.toml"));
    }

    #[test]
    fn quick_setup_homebrew_service_note_skips_matching_service_layout() {
        let service_config = Path::new("/opt/homebrew/var/opsclaw/config.toml");
        let service_workspace = Path::new("/opt/homebrew/var/opsclaw/workspace");
