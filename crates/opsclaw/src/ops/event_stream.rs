...
impl StreamEvent {
    /// Return a human-readable identifier for the source of this event.
    ///
    /// For Docker events this is the container name; for systemd events it is
    /// the unit name.
    pub fn source_name(&self) -> &str {
        match self {
            StreamEvent::Docker(ev) => &ev.actor_name,
            StreamEvent::Systemd(ev) => &ev.unit,
        }
    }
}
