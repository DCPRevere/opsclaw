...
impl WebhookNotifier {
    pub fn new(url: String, bearer_token: Option<String>, min_severity: AlertSeverity) -> Self {
        Self {
            url,
            bearer_token,
            min_severity,
    ...
    async fn post_json(&self, payload: &serde_json::Value) -> anyhow::Result<()> {
...
impl AlertNotifier for WebhookNotifier {
    ...
    async fn notify(&self, target_name: &str, health: &HealthCheck) -> anyhow::Result<()> {
        let relevant: Vec<serde_json::Value> = health
            .alerts
            .iter()
            .filter(|a| severity_meets_threshold(&a.severity, &self.min_severity))
            .map(|a| {
