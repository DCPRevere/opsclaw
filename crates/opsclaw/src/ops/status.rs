...
use zeroclaw::config::schema::TargetConfig;

/// Collected data for a single target's status display.
...
/// Build a `TargetStatus` from config + snapshot on disk.
...
/// Render a human-readable status report for a single target.
pub fn render_status(status: &TargetStatus) -> String {
