...
impl SshExecutor for RealSshExecutor {
    async fn run(
        ...
        let result = tokio::time::timeout(timeout, self.run_inner(project, command, pty)).await;
        match result {
            Ok(inner) => inner,
...
impl RealSshExecutor {
    async fn run_inner(
        ...
        let key_pair = russh_keys::decode_secret_key(&project.private_key_pem, None)
        ...
        let mut handle = russh::client::connect(config, (&*project.host, project.port), handler)
            .await
            .map_err(|e| {
                anyhow::anyhow!(
        ...
        let authenticated = handle
            .authenticate_publickey(&project.user, Arc::new(key_pair))
            .await
...
impl Tool for SshTool {
    ...
    async fn execute(&self, args: serde_json::Value) -> anyhow::Result<ToolResult> {
        ...
        // Resolve project.
        let target = match self.resolve_project(target_name) {
            Some(t) => t,
            None => {
...
mod tests {
    ...
    async fn unknown_target_returns_error() {
        let tool = SshTool::new(config_with(vec![default_project(OpsClawAutonomy::Auto)]));
        let result = tool
    ...
    fn audit_log_format() {
        ...
        assert!(log.contains("PROJECT=web-1"));
