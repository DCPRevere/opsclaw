...
fn context_returns_configured_targets() {
    ...
    let ctx = OpsClawContext::new(config);
...
#[test]
fn ssh_runner_for_resolves_key_from_config() {
    let key_content =
        "-----BEGIN OPENSSH PRIVATE KEY-----\ntest-key-data\n-----END OPENSSH PRIVATE KEY-----";
    let mut target = sacra_target();
    target.key_secret = Some(key_content.to_string());
...
#[test]
fn ssh_runner_for_missing_key_secret_errors() {
    let mut target = sacra_target();
    target.key_secret = None;
    let config = test_config(vec![target]);
    let ctx = OpsClawContext::new(config);

    let err = ctx.ssh_runner_for("sacra").unwrap_err();
    assert!(err.to_string().contains("missing key_secret"));
}
