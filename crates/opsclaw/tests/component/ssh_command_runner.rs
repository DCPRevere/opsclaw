use opsclaw::tools::discovery::CommandRunner;
use opsclaw::tools::ssh_command_runner::{LocalCommandRunner, SshCommandRunner};
use opsclaw::tools::ssh_tool::{OpsClawAutonomy, SshExecutor, SshOutput, TargetEntry};
...
async fn local_writes_audit_log() {
    let tmp = tempfile::tempdir().unwrap();
    let runner = LocalCommandRunner::new(OpsClawAutonomy::Auto, "sidecar".into())
        .with_audit_dir(tmp.path().to_path_buf());
...
async fn local_timeout_is_enforced() {
    let runner =
        LocalCommandRunner::new(OpsClawAutonomy::Auto, "local".into()).with_timeout_secs(1);
