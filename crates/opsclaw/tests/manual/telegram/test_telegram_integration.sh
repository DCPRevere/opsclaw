#!/bin/bash
# OpsClaw Telegram Integration Test Suite
...
if [ -f "target/release/opsclaw" ]; then
    BINARY_SIZE=$(ls -lh target/release/opsclaw | awk '{print $5}')
    SIZE_BYTES=$(stat -f%z target/release/opsclaw 2>/dev/null || stat -c%s target/release/opsclaw)
...
CONFIG_PATH="$HOME/.opsclaw/config.toml"
if [ -f "$CONFIG_PATH" ]; then
    ...
    if grep -q "\[channels_config.telegram\]" "$CONFIG_PATH"; then
        ...
        warn "Telegram not configured - run 'opsclaw onboard' first"
    fi
else
    warn "No config file found - run 'opsclaw onboard' first"
fi
...
START_TIME=$(date +%s)
HEALTH_OUTPUT=$(timeout 10 target/release/opsclaw channel doctor 2>&1 || true)
END_TIME=$(date +%s)
