...
This guide covers testing the Telegram channel integration for OpsClaw.
...
6. **Error logging**
    ...
    ```bash
    RUST_LOG=debug opsclaw channel start
    ```
...
Solution: Check user allowlist
  ...
  4. Run: opsclaw onboard --channels-only
```
...
```bash
# Maximum logging
RUST_LOG=trace opsclaw channel start
...
# Validate config
cat ~/.opsclaw/config.toml | grep -A 3 "\[channels_config.telegram\]"
```
...
- [OpsClaw Main README](../../README.md)
- [Contributing Guide](../../CONTRIBUTING.md)
- [Issue Tracker](https://github.com/DCPRevere/opsclaw/issues)
