...
Complete guide to setting up an OpsClaw-powered robot on Raspberry Pi 5.
...
mkdir -p ~/.opsclaw/models
cp models/ggml-base.bin ~/.opsclaw/models/
```
...
mkdir -p ~/.opsclaw/models/piper
cd ~/.opsclaw/models/piper
wget https://huggingface.co/rhasspy/piper-voices/resolve/main/en/en_US/lessac/medium/en_US-lessac-medium.onnx
wget https://huggingface.co/rhasspy/piper-voices/resolve/main/en/en_US/lessac/medium/en_US-lessac-medium.onnx.json

# Test
echo "Hello, I am your robot!" | piper --model ~/.opsclaw/models/piper/en_US-lessac-medium.onnx --output_file test.wav
aplay test.wav
```
...
### 7. Build OpsClaw Robot Kit

```bash
# Clone repo (or copy from USB)
git clone https://github.com/dcprevere/opsclaw
cd opsclaw

# Build robot kit
cargo build --release -p opsclaw-robot-kit

# Build main opsclaw (optional, if using as agent)
cargo build --release
```
...
```bash
mkdir -p ~/.opsclaw
nano ~/.opsclaw/robot.toml
```

```toml
# ~/.opsclaw/robot.toml - Real Hardware Configuration
...
# Test speaker
echo "Testing speaker" | piper --model ~/.opsclaw/models/piper/en_US-lessac-medium.onnx --output_file - | aplay -D plughw:0,0
...
FIFO_PATH = "/tmp/opsclaw_sensors.fifo"
...
```bash
# Configure OpsClaw to use robot tools
cat > ~/.opsclaw/config.toml << 'EOF'
...
cp ~/opsclaw/crates/robot-kit/SOUL.md ~/.opsclaw/workspace/

# Start agent
./target/release/opsclaw agent
```
...
# Start sensor loop
if [ ! -p /tmp/opsclaw_sensors.fifo ]; then
    mkfifo /tmp/opsclaw_sensors.fifo
fi
...
```bash
# /etc/systemd/system/opsclaw-robot.service
sudo tee /etc/systemd/system/opsclaw-robot.service << 'EOF'
[Unit]
Description=OpsClaw Robot
After=network.target ollama.service

[Service]
Type=simple
User=pi
WorkingDirectory=/home/pi/opsclaw
...
sudo systemctl enable opsclaw-robot
sudo systemctl start opsclaw-robot

# Check status
sudo systemctl status opsclaw-robot
journalctl -u opsclaw-robot -f  # View logs
```
