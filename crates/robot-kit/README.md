...
A complete toolkit for building AI-powered robots with OpsClaw. Designed for Raspberry Pi deployment with offline Ollama inference.
...
### 1. Build OpsClaw with robot tools

```bash
# Clone and build
git clone https://github.com/DCPRevere/opsclaw
cd opsclaw
cargo build -p opsclaw-robot-kit --release
```
...
# Copy config
mkdir -p ~/.opsclaw
cp crates/robot-kit/robot.toml ~/.opsclaw/
cp crates/robot-kit/SOUL.md ~/.opsclaw/workspace/
...
# Test in mock mode
./target/release/opsclaw agent -m "Say hello and show a happy face"

# Test with real hardware
# (after configuring robot.toml)
./target/release/opsclaw agent -m "Move forward 1 meter"
```
...
```rust
use opsclaw_robot_kit::{create_tools, RobotConfig};
...
If you want runtime registration in `opsclaw`, add a thin adapter that maps this
...
mkdir opsclaw-robot-kit
cp -r target/release/opsclaw opsclaw-robot-kit/
cp -r examples/robot_kit opsclaw-robot-kit/
cp -r ~/.opsclaw opsclaw-robot-kit/dot-opsclaw

# Include models
mkdir -p opsclaw-robot-kit/models
cp ~/.opsclaw/models/ggml-base.bin opsclaw-robot-kit/models/
# Note: Ollama models are large, may want to download on target

# Create tarball
tar -czvf opsclaw-robot-kit.tar.gz opsclaw-robot-kit/

# Copy to USB
cp opsclaw-robot-kit.tar.gz /media/usb/TarBalls/
```
