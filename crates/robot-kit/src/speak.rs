...
impl SpeakTool {
    pub fn new(config: RobotConfig) -> Self {
        let audio_dir = directories::UserDirs::new()
            .map(|d| d.home_dir().join(".opsclaw/tts_cache"))
            .unwrap_or_else(|| PathBuf::from("/tmp/opsclaw_tts"));
    ...
    async fn speak(&self, text: &str, emotion: &str) -> Result<()> {
        ...
        let model_path = directories::UserDirs::new()
            .map(|d| {
                d.home_dir()
                    .join(format!(".opsclaw/models/piper/{}.onnx", voice))
            })
    ...
    async fn play_sound(&self, sound: &str) -> Result<()> {
        let sounds_dir = directories::UserDirs::new()
            .map(|d| d.home_dir().join(".opsclaw/sounds"))
            .unwrap_or_else(|| PathBuf::from("/usr/local/share/opsclaw/sounds"));
