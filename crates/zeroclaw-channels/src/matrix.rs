...
mod client {
        ...
        if recovery_attempts > 1 {
            bail!(
                "matrix: corruption recovery looped — aborting to avoid an infinite restart cycle. \
                 Wipe ~/.opsclaw/state/matrix/ manually and restart."
            );
        }
    ...
    async fn password_login(client: &Client, config: &MatrixConfig, password: &str) -> Result<()> {
        ...
        let mut login = client
            ...
            .initial_device_display_name("OpsClaw");
    ...
    async fn access_token_login(client: &Client, config: &MatrixConfig) -> Result<()> {
        ...
        let device_id = config
            ...
            .clone()
            .filter(|s| !s.is_empty())
            .unwrap_or_else(|| format!("OPSCLAW_{}", uuid::Uuid::new_v4().simple()));
        let session = MatrixSession {
            meta: SessionMeta {
                user_id,
...
mod inbound {
    ...
    async fn save_media_to_workspace(
        ...
        let Some(workspace) = workspace else {
            warn!(
                "matrix: cannot persist {} — channels.matrix workspace_dir not configured. Set OPSCLAW_DIR or run via the orchestrator.",
                info.file_name
            );
            return Ok(None);
        };
...
mod tests {
    ...
    mod mention {
        ...
        fn body_fallback_display_name() {
            let bot = user_id!("@bot:example.org");
            assert!(is_mentioned(bot, Some("OpsClaw"), None, "hi opsclaw!"));
        }

        #[test]
        fn no_mention_rejects() {
            let bot = user_id!("@bot:example.org");
            assert!(!is_mentioned(bot, Some("OpsClaw"), None, "no mention here"));
        }
    }
