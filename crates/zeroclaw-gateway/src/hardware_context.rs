...
//!
//! OpsClaw's agent loop calls [`zeroclaw_hardware::boot`] on **every** request,
//! which re-reads `~/.opsclaw/hardware/` from disk.  Writing to those files
...
/// Return `~/.opsclaw/hardware/` or an error string.
fn hardware_dir() -> Result<PathBuf, String> {
    directories::BaseDirs::new()
        .map(|b| b.home_dir().join(".opsclaw").join("hardware"))
...
/// `POST /api/hardware/pin` — register a single GPIO pin assignment.
///
/// Appends one line to `~/.opsclaw/hardware/devices/<device>.md`:
/// ```text
