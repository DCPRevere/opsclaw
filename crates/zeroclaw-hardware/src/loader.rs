//! Plugin manifest loader — scans `~/.opsclaw/tools/` at startup.
//!
...
/// Scan `~/.opsclaw/tools/` and return all valid plugins.
///
...
pub fn scan_plugin_dir() -> Vec<LoadedPlugin> {
    ...
    println!(
        ...
        match dirs_home().as_deref().filter(|s| !s.is_empty()) {
            Some(home) => tools_dir
                .to_str()
                .unwrap_or("~/.opsclaw/tools")
                .replace(home, "~"),
            None => tools_dir.to_str().unwrap_or("~/.opsclaw/tools").to_string(),
        }
    );
...
"#;
    ...
    fn scan_plugin_dir_skips_broken_manifests_without_panicking() {
        ...
        // always uses ~/.opsclaw/tools), but we can verify load_one_plugin
    ...
    fn plugin_tools_dir_returns_path_ending_in_zeroclaw_tools() {
        ...
        let expected = std::path::Path::new(".opsclaw").join("tools");
