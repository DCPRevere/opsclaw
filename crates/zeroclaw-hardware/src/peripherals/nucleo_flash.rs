//! Flash OpsClaw Nucleo-F401RE firmware via probe-rs.
//!
...
/// Flash OpsClaw Nucleo firmware. Builds from firmware/nucleo.
pub fn flash_nucleo_firmware() -> Result<()> {
    ...
    // CARGO_MANIFEST_DIR = crate dir; workspace root is two levels up.
    let repo_root = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../..");
    let firmware_dir = repo_root.join("firmware").join("nucleo");
    if !firmware_dir.join("Cargo.toml").exists() {
        anyhow::bail!(
            "Nucleo firmware not found at {}. Run from opsclaw repo root.",
            firmware_dir.display()
        );
    }

    println!("Building OpsClaw Nucleo firmware...");
    ...
    println!("OpsClaw Nucleo firmware flashed successfully.");
