...
//!
//! Wire protocol (OpsClaw serial JSON):
//! ```text
...
/// Default baud rate for OpsClaw serial devices.
...
/// Serial transport for OpsClaw hardware devices.
///
...
impl HardwareSerialTransport {
    ...
    /// Attempt a ping handshake to verify OpsClaw firmware is running.
    ///
    /// Opens the port, sends `{"cmd":"ping","params":{}}`, waits up to
    /// `PING_TIMEOUT_MS` for a response with `data.firmware == "zeroclaw"`.
    ///
    /// Returns `true` if a OpsClaw device responds, `false` otherwise.
