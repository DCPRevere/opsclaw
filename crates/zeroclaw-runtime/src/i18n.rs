...
///
/// Checks `OPSCLAW_LOCALE`, then `LANG`, then `LC_ALL`.
/// Returns "en" if none are set or parseable.
pub fn detect_locale() -> String {
    if let Ok(val) = std::env::var("OPSCLAW_LOCALE") {
...
"#,
    ...
    fn detect_locale_from_env() {
        ...
        let saved = std::env::var("OPSCLAW_LOCALE").ok();
        let saved_lang = std::env::var("LANG").ok();

        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("OPSCLAW_LOCALE", "ja-JP") };
        assert_eq!(detect_locale(), "ja-JP");

        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::remove_var("OPSCLAW_LOCALE") };
        // SAFETY: test-only, single-threaded test runner.
        ...
        match saved {
            // SAFETY: test-only, single-threaded test runner.
            Some(v) => unsafe { std::env::set_var("OPSCLAW_LOCALE", v) },
            // SAFETY: test-only, single-threaded test runner.
            None => unsafe { std::env::remove_var("OPSCLAW_LOCALE") },
        }
        match saved_lang {
            // SAFETY: test-only, single-threaded test runner.
