...
/// - `X-OpsClaw-Timestamp` — unix epoch seconds
/// - `X-OpsClaw-Nonce` — random UUID v4
/// - `X-OpsClaw-Signature` — HMAC-SHA256 hex digest
///
...
impl NodeTransport {
    ...
    pub async fn send(
        ...
        let resp = self
            .http
            .post(&url)
            .header("X-OpsClaw-Timestamp", timestamp.to_string())
            .header("X-OpsClaw-Nonce", &nonce)
            .header("X-OpsClaw-Signature", &signature)
