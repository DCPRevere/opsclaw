...
///
/// Resolution order: `ZEROCLAW_WORKSPACE` env → `active_workspace.toml` marker → `~/.opsclaw/config.toml`.
...
pub struct Config {
    ...
    /// OpsClaw LLM diagnosis settings (`[diagnosis]`).
    #[serde(default)]
...
// ── OpsClaw diagnosis configuration ─────────────────────────────
...
pub struct DiagnosisConfig {
    /// API key for the diagnosis LLM provider. Falls back to `ANTHROPIC_API_KEY` env var.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub api_key: Option<String>,
    /// Model to use for diagnosis. Falls back to `OPSCLAW_DIAGNOSIS_MODEL` env var.
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
/// Configuration for the A2A server (shared between config and a2a_server tool).
#[derive(Debug, Clone, Serialize, Deserialize, JsonSchema)]
pub struct A2aServerConfig {
    /// Enable the A2A server (default: false).
    #[serde(default)]
    pub enabled: bool,
...
pub struct A2aPeer {
    ...
    pub name: String,
    /// Base URL of the peer's A2A server.
    pub url: String,
    /// Bearer token for authenticating to this peer.
    #[serde(default)]
    pub token: String,
}
...
"#;
    ...
    async fn parse_extra_headers_env_with_url_value() {
        ...
        assert_eq!(headers[0].1, "https://github.com/DCPRevere/opsclaw");
    }
