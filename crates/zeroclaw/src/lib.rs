...
Retrieves the agent card published at `/.well-known/agent-card.json` \
and displays the agent's name, description, skills, and auth scheme.
...
Examples:
  opsclaw a2a discover https://agent.example.com")]
    ...
    /// Send a task to a configured peer
    #[command(long_about = "\
Send a task message to a named A2A peer.

The peer must be configured in `[a2a.peers]` in config.toml.

Examples:
  opsclaw a2a send my-peer 'Summarize the latest logs'")]
    Send {
        /// Peer name (from config)
        peer: String,
        /// Message to send
        message: String,
    },
...
Listens for incoming A2A JSON-RPC requests and serves the agent card. \
Bind address and port are taken from `[a2a.server]` in config.toml.

Examples:
  opsclaw a2a server start
  opsclaw a2a server start")]
    Server,
