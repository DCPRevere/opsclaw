...
fn main() {
    // build.rs runs with cwd = CARGO_MANIFEST_DIR (crates/zeroclawlabs/).
    // web/ and docs/ live at the workspace root, two levels up.
    let root = Path::new("../..");
...
fn ensure_dashboard_assets(dist_dir: &Path) {
    let root = Path::new("../..");
    let src = root.join("docs/assets/zeroclaw-trans.png");
    if !src.exists() {
        eprintln!(
