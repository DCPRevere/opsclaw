    ...
    let defaults: &[(&str, &str)] = &[
        (
            ...
            "# IDENTITY.md — Who Am I?\n\n\
             I am OpsClaw, an autonomous SRE agent.\n\n\
             ## Traits\n\
...
impl Config {
    pub async fn load_or_init() -> Result<Self> {
        ...
        if config_path.exists() {
            ...
            // Top-level keys owned by the opsclaw application layer (not the
            // zeroclawlabs library).  They are present in the shared config.toml
            // but unknown to the zeroclaw schema — suppress false-positive warnings.
            const OPSCLAW_EXTENSION_PREFIXES: &[&str] =
                &["projects", "diagnosis", "a2a", "notifications"];
            ...
            for path in ignored_paths {
                ...
                // Skip keys belonging to the opsclaw extension layer.
                let root = path.split('.').next().unwrap_or(&path);
                if OPSCLAW_EXTENSION_PREFIXES.contains(&root) {
                    continue;
                }
                tracing::warn!(
