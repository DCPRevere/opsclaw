# OpsClaw Development Environment

A fully containerized development sandbox for OpsClaw agents. This environment allows you to develop, test, and debug the agent in isolation without modifying your host system.
...
### 2. Enter Agent Container (`opsclaw-dev`)

```bash
./dev/cli.sh agent
```

Use this to run `opsclaw` CLI commands manually, debug the binary, or check logs internally.
...
- **Agent**: `/opsclaw-data/workspace`
- **Sandbox**: `/home/developer/workspace`

Files created by the agent are visible to the sandbox user, and vice versa.

The agent configuration lives in `target/.opsclaw` (mounted to `/opsclaw-data/.opsclaw`), so settings persist across container rebuilds.
...
**Note:** This removes `target/.opsclaw` (config/DB) but leaves the `playground/` directory intact. To fully wipe everything, manually delete `playground/`.
...
- Rust compilation, tests, and audit/deny tools run in `opsclaw-local-ci` container.
...
- The root `Dockerfile` also caches Rust `target/` (`id=opsclaw-target`) to speed repeat local image builds.
