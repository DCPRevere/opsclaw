...
ASSET_URL="https://github.com/DCPRevere/opsclaw/releases/download/${TAG}/${ASSET_NAME}"
TEMP_DIR="$(mktemp -d -t opsclaw-termux-test-XXXXXX)"
...
# --- Test 5: Contains opsclaw binary ---
info "Checking archive contents"
CONTENTS=$(tar -tzf "$TEMP_DIR/$ASSET_NAME")
if echo "$CONTENTS" | grep -q "^opsclaw$"; then
  pass "Archive contains 'opsclaw' binary"
else
  fail "Archive does not contain 'opsclaw' binary"
  echo "Contents:"
  echo "$CONTENTS" | sed 's/^/  /'
fi
...
CHECKSUMS_URL="https://github.com/DCPRevere/opsclaw/releases/download/${TAG}/SHA256SUMS"
