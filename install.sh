...
if [ "$SKIP_ONBOARD" = false ] && [ "$DRY_RUN" != true ] && [ -f "$BIN" ]; then
  ...
  if ! onboarding_needed; then
    ...
    echo
    printf "%s\n" "$(bold "OpsClaw installed. How would you like to complete onboarding?")"
    printf "  [1] CLI/TUI  (opsclaw onboard)\n"
    printf "  [2] Open gateway in browser (opsclaw daemon + dashboard)\n"
    ...
    case "${onboard_choice:-1}" in
      ...
      2)
        ...
        info "Stop the daemon with Ctrl+C when done; then run 'opsclaw service install' for always-on."
        "$BIN" daemon || warn "Daemon exited with an error — run 'opsclaw daemon' manually"
        ;;
      3)
        info "Skipped onboarding. Run 'opsclaw onboard' (CLI) or 'opsclaw daemon' (browser) when ready."
        ;;
      *)
        warn "Unknown choice '$onboard_choice' — skipping. Run 'opsclaw onboard' to configure."
        ;;
    esac
  else
    info "Non-interactive — skipping onboard prompt. Run 'opsclaw onboard' to configure."
  fi
fi
