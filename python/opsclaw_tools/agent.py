...
SYSTEM_PROMPT = """You are OpsClaw, an AI assistant with tool access. Use tools to accomplish tasks.
...
class OpsclawAgent:
    ...
    def invoke(self, input: dict[str, Any], config: Optional[dict] = None) -> dict:
        ...
        raise RuntimeError(
            "OpsclawAgent.invoke() cannot be called inside an active event loop. "
            "Use 'await OpsclawAgent.ainvoke(...)' instead."
        )
...
def create_agent(
    ...
    """
    Create an OpsClaw agent with LangGraph-based tool calling.
    ...
    Returns:
        Configured OpsclawAgent instance

    Example:
        ```python
        from opsclaw_tools import create_agent, shell, file_read
