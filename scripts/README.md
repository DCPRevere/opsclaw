...
This directory contains everything needed to cross-compile OpsClaw and deploy it to a Raspberry Pi over SSH.
...
| `rpi-config.toml` | Production config template deployed to `~/.opsclaw/config.toml` |
| `opsclaw.service` | systemd unit file installed on the Pi |
...
| `RPI_DIR` | `~/opsclaw` | Remote directory for the binary and `.env` |
...
2. **Stop service** — runs `sudo systemctl stop opsclaw` on the Pi (continues if not yet installed).
3. **Create remote directory** — ensures `$RPI_DIR` exists on the Pi.
4. **Copy binary** — SCPs the compiled binary to `$RPI_DIR/opsclaw`.
...
6. **Deploy config** — copies `rpi-config.toml` to `~/.opsclaw/config.toml`, preserving any `api_key` already present in the file.
7. **Install systemd service** — copies `opsclaw.service` to `/etc/systemd/system/`, then enables and restarts it.
...
sudo systemctl restart opsclaw
```
...
## Interacting with OpsClaw on the Pi
...
```bash
ssh pi@raspberrypi.local 'journalctl -u opsclaw -f'
```
...
OpsClaw is deployed with the `peripheral-rpi` feature, which enables two LLM-callable tools:
...
|------------|--------|-------------|
| `~/opsclaw/opsclaw` | compiled binary | Main agent binary |
| `~/opsclaw/.env` | created on first deploy | API key and environment variables |
| `~/.opsclaw/config.toml` | `rpi-config.toml` | Agent configuration |
| `/etc/systemd/system/opsclaw.service` | `opsclaw.service` | systemd service unit |
...
To customise, edit `~/.opsclaw/config.toml` directly on the Pi and restart the service.
...
```bash
ssh pi@raspberrypi.local 'sudo systemctl status opsclaw'
ssh pi@raspberrypi.local 'journalctl -u opsclaw -n 50 --no-pager'
```
...
```bash
ssh pi@raspberrypi.local 'file ~/opsclaw/opsclaw'
