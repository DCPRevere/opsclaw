#!/usr/bin/env bash
# deploy-rpi.sh — cross-compile OpsClaw for Raspberry Pi and deploy via SSH.
#
...
RPI_DIR="${RPI_DIR:-/home/${RPI_USER}/opsclaw}"
...
echo "==> Building OpsClaw for Raspberry Pi (${TARGET})"
...
echo ""
echo "==> Stopping opsclaw service (if running)"
# shellcheck disable=SC2029
${SSH_CMD} ${SSH_OPTS} "${RPI_USER}@${RPI_HOST}" \
  "sudo systemctl stop opsclaw 2>/dev/null || true"
...
# ── 4. Deploy binary ──────────────────────────────────────────────────────────
echo ""
echo "==> Deploying binary to ${RPI_USER}@${RPI_HOST}:${RPI_DIR}/opsclaw"
${SCP_CMD} ${SCP_OPTS} "${BINARY}" "${RPI_USER}@${RPI_HOST}:${RPI_DIR}/opsclaw"
...
# ── 5. Deploy config ─────────────────────────────────────────────────────────
CONFIG_DEST="/home/${RPI_USER}/.opsclaw/config.toml"
echo ""
echo "==> Deploying config to ${CONFIG_DEST}"
# shellcheck disable=SC2029
${SSH_CMD} ${SSH_OPTS} "${RPI_USER}@${RPI_HOST}" "mkdir -p /home/${RPI_USER}/.opsclaw"
...
SERVICE_DEST="/etc/systemd/system/opsclaw.service"
echo ""
echo "==> Installing systemd service (requires sudo on the Pi)"
${SCP_CMD} ${SCP_OPTS} "scripts/opsclaw.service" "${RPI_USER}@${RPI_HOST}:/tmp/opsclaw.service"
# shellcheck disable=SC2029
${SSH_CMD} ${SSH_OPTS} "${RPI_USER}@${RPI_HOST}" \
  "sudo mv /tmp/opsclaw.service ${SERVICE_DEST} && \
   sudo systemctl daemon-reload && \
   sudo systemctl enable opsclaw && \
   sudo systemctl restart opsclaw && \
   sudo systemctl status opsclaw --no-pager || true"
...
# ── 8. Reset ACT LED trigger so OpsClaw can control it ──────────────────────
echo ""
...
echo ""
echo "    OpsClaw is running at http://${RPI_HOST}:8080"
echo "    POST /api/chat  — chat with the agent"
echo "    GET  /health    — health check"
echo ""
echo "    To check logs: ssh ${RPI_USER}@${RPI_HOST} 'journalctl -u opsclaw -f'"
