...
:: ============================================================================
:: OpsClaw Windows Setup Script
:: Simplifies building and installing OpsClaw on Windows.
...
set "REPO=https://github.com/dcprevere/opsclaw"
...
echo %BOLD%%BLUE%  OpsClaw Windows Setup  v%VERSION%%RESET%
echo %BOLD%%BLUE%=========================================%RESET%
echo.
...
if not defined DOWNLOAD_URL (
    ...
    set "DOWNLOAD_URL=https://github.com/dcprevere/opsclaw/releases/latest/download/opsclaw-%TARGET%.zip"
)

echo   Downloading from release...
curl -sSfL -o "%TEMP%\opsclaw-windows.zip" "!DOWNLOAD_URL!"
if %ERRORLEVEL% NEQ 0 (
...
:: Extract
echo   Extracting...
mkdir "%USERPROFILE%\.opsclaw\bin" 2>nul
tar -xf "%TEMP%\opsclaw-windows.zip" -C "%USERPROFILE%\.opsclaw\bin"
if %ERRORLEVEL% NEQ 0 (
    powershell -Command "Expand-Archive -Force '%TEMP%\opsclaw-windows.zip' '%USERPROFILE%\.opsclaw\bin'"
)

:: Add to PATH if not already there
echo %PATH% | findstr /I /C:".opsclaw\bin" >nul 2>&1
if %ERRORLEVEL% NEQ 0 (
    setx PATH "%PATH%;%USERPROFILE%\.opsclaw\bin" >nul 2>&1
    set "PATH=%PATH%;%USERPROFILE%\.opsclaw\bin"
    echo   %GREEN%OK%RESET% Added to PATH
)

echo   %GREEN%OK%RESET% Binary installed to %USERPROFILE%\.opsclaw\bin\opsclaw.exe
goto verify
...
:do_build
echo.
echo %BOLD%[3/5] Building OpsClaw (%BUILD_DESC%)...%RESET%
echo   Target: %TARGET%

:: Ensure we're in the repo root (check for Cargo.toml)
if not exist "Cargo.toml" (
    echo   %RED%ERROR: Cargo.toml not found. Run this script from the opsclaw repository root.%RESET%
    echo   Example:
    echo     git clone %REPO%
    echo     cd opsclaw
    echo     setup.bat
...
mkdir "%USERPROFILE%\.opsclaw\bin" 2>nul
copy /Y "target\%TARGET%\release\opsclaw.exe" "%USERPROFILE%\.opsclaw\bin\opsclaw.exe" >nul
echo   %GREEN%OK%RESET% Installed to %USERPROFILE%\.opsclaw\bin\opsclaw.exe
...
"%USERPROFILE%\.opsclaw\bin\opsclaw.exe" --version >nul 2>&1
if %ERRORLEVEL% EQU 0 (
    for /f "tokens=*" %%v in ('"%USERPROFILE%\.opsclaw\bin\opsclaw.exe" --version 2^>nul') do (
        echo   %GREEN%OK%RESET% %%v
    )
) else (
    opsclaw --version >nul 2>&1
    if %ERRORLEVEL% EQU 0 (
        for /f "tokens=*" %%v in ('opsclaw --version 2^>nul') do (
            echo   %GREEN%OK%RESET% %%v
        )
    ) else (
...
echo %BOLD%%GREEN%  OpsClaw setup complete!%RESET%
echo %BOLD%%GREEN%=========================================%RESET%
echo.
echo   Next steps:
echo     1. Restart your terminal (for PATH changes)
echo     2. Run: opsclaw onboard
echo     3. Configure your API key in %%USERPROFILE%%\.opsclaw\config.toml
echo.
echo   Alternative install via Scoop:
echo     scoop bucket add opsclaw https://github.com/dcprevere/scoop-opsclaw
echo     scoop install opsclaw
echo.
echo   Documentation: https://github.com/dcprevere/opsclaw
echo.
goto :end
...
:show_help
echo.
echo OpsClaw Windows Setup Script
echo.
