...
//!
//! Both enforce the observe-mode read-only allowlist and write to the
...
use super::ssh_tool::{
    is_read_only_command, write_audit_entry, OpsClawAutonomy, SshExecutor, TargetEntry,
};
...
impl SshCommandRunner {
    pub fn new(target: TargetEntry, executor: Box<dyn SshExecutor>) -> Self {
        Self {
...
impl CommandRunner for SshCommandRunner {
    async fn run(&self, command: &str) -> Result<CommandOutput> {
        ...
        if self.target.autonomy == OpsClawAutonomy::Observe {
            if let Err(reason) = is_read_only_command(command) {
                bail!("observe mode: {reason}");
            }
        }
        ...
        let result = tokio::time::timeout(
            self.timeout,
            self.executor
                .run(&self.target, command, self.timeout, false),
        )
        .await;
...
impl LocalCommandRunner {
    pub fn new(autonomy: OpsClawAutonomy, target_name: String) -> Self {
        Self {
            autonomy,
            target_name,
            timeout_secs: 30,
...
impl CommandRunner for LocalCommandRunner {
    async fn run(&self, command: &str) -> Result<CommandOutput> {
        // Autonomy enforcement.
        if self.autonomy == OpsClawAutonomy::Observe {
            if let Err(reason) = is_read_only_command(command) {
                bail!("observe mode: {reason}");
            }
        }
...
mod tests {
    ...
    fn test_target(autonomy: OpsClawAutonomy) -> TargetEntry {
        TargetEntry {
            name: "test-host".into(),
            host: "127.0.0.1".into(),
            port: 22,
    ...
    async fn ssh_runner_observe_rejects_write() {
        let runner = SshCommandRunner::new(
            test_target(OpsClawAutonomy::Observe),
    ...
    async fn ssh_runner_timeout() {
        let runner = SshCommandRunner::new(
            test_target(OpsClawAutonomy::FullAuto),
